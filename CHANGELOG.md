...
- Turn sequencing Phase 1.5: Added legacy active player projection compatibility in `TurnAdvanceStateMutator` – when advancing past final segment it now rotates `ActivePlayerState` (if present) alongside incrementing turn number and resetting segment. Added rotation test (guarded if no active player states present in module). Backlog Workstream 10 updated to mark projection layer complete; next steps: integrate Backgammon doubling cube gating via TurnState and introduce pass/commit events.
- Backgammon doubling cube gating (Phase 2 – abstraction correction): Removed `LastDoubledTurn` from core `TurnState` (leaky Backgammon-specific concern). Introduced `DoublingDiceState.LastDoubledTurn` and updated `DoublingDiceStateMutator` to block same-turn redoubles (value remains unchanged) while preserving future multi-turn redouble path (will leverage segment-specific gating once Roll segment introduced). Updated property tests (`BackgammonInvariants`) to assert same-turn redouble unchanged and owner-stability; unskipped previous placeholders. Backlog & docs updated to reflect domain-specific relocation.
- Backgammon multi-turn redouble gating (Workstream 10 advancement): Implemented multi-turn doubling sequence 2→4→8 with explicit turn advancement requirement. Updated `DoublingDiceStateMutator` to enforce owner-on-roll redouble rule and ownership transfer to opponent upon successful doubling; updated `DoublingDiceWithActivePlayerGameEventCondition` to allow redouble only when `TurnState.TurnNumber > DoublingDiceState.LastDoubledTurn` and the cube owner is the active player (presence of `TurnState` as sequencing signal). Added property test `GivenDistinctTurns_WhenOpponentsAlternateRedoubles_ThenValueProgresses2xEachTurn` validating progression and ownership alternation. Removed prior backlog deferral for multi-turn redoubles; backlog-next & action-plan updated. Style charter reaffirmed (file-scoped namespaces, explicit braces, no hot-path LINQ, immutable state, deterministic transitions) with no deviations.
