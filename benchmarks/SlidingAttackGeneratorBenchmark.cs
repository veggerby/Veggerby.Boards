...
public class SlidingAttackGeneratorBenchmark
{
    ...
    private static BoardShape BuildRing(int n)
    {
        ...
        for (int i = 0; i < n; i++) tiles.Add(new Tile($"t{i}"));
        var east = new Direction("east");
        var rels = new List<TileRelation>(n);
        for (int i = 0; i < n - 1; i++) rels.Add(new TileRelation(tiles[i], tiles[i + 1], east));
    ...
    private static BoardShape BuildGrid(bool orthogonalOnly)
    {
        ...
        foreach (var f in files)
        {
            for (int r = 1; r <= 8; r++)
            {
                ...
                if (!orthogonalOnly)
                {
                    if (r < 8 && f != 'h') rels.Add(new TileRelation(T(f, r), T((char)(f + 1), r + 1), ne));
                    if (r < 8 && f != 'a') rels.Add(new TileRelation(T(f, r), T((char)(f - 1), r + 1), nw));
                    if (r > 1 && f != 'h') rels.Add(new TileRelation(T(f, r), T((char)(f + 1), r - 1), se));
                    if (r > 1 && f != 'a') rels.Add(new TileRelation(T(f, r), T((char)(f - 1), r - 1), sw));
                }
            }
        }
