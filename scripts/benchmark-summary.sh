...
# 
# Regenerates docs/performance/benchmark-summary.md from BenchmarkDotNet results.
# Extracts metadata (commit, date, .NET version, OS) and key metrics.
#
# Usage:
#   ./scripts/benchmark-summary.sh
#
# Prerequisites:
#   - Run benchmarks first: ./scripts/run-all-benchmarks.sh
#   - BenchmarkDotNet.Artifacts directory with JSON results
#
# Output:
#   - Updates docs/performance/benchmark-summary.md with latest metadata and metrics

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT_FILE="$ROOT_DIR/docs/performance/benchmark-summary.md"
...
echo "[benchmark-summary] Generating benchmark summary..." >&2
echo "[benchmark-summary]   Commit: $COMMIT" >&2
echo "[benchmark-summary]   Date: $DATE" >&2
echo "[benchmark-summary]   .NET: $DOTNET_VERSION" >&2
echo "[benchmark-summary]   OS: $OS_NAME ($ARCH)" >&2

# Check if benchmark results exist
ARTIFACTS_DIR="$ROOT_DIR/BenchmarkDotNet.Artifacts/results"
if [[ ! -d "$ARTIFACTS_DIR" ]]; then
    echo "[benchmark-summary] ERROR: No benchmark artifacts found at $ARTIFACTS_DIR" >&2
    echo "[benchmark-summary] Run benchmarks first: ./scripts/run-all-benchmarks.sh" >&2
    exit 1
fi

# Count available benchmark result files
RESULT_COUNT=$(find "$ARTIFACTS_DIR" -name "*.json" 2>/dev/null | wc -l || echo "0")
if [[ "$RESULT_COUNT" -eq 0 ]]; then
    echo "[benchmark-summary] WARNING: No JSON benchmark results found in $ARTIFACTS_DIR" >&2
    echo "[benchmark-summary] Run benchmarks first: ./scripts/run-all-benchmarks.sh" >&2
fi

# Create temporary file for new summary
TEMP_FILE=$(mktemp)
...
**Last Updated**: $DATE  
**Commit**: \`$COMMIT\`  
**Environment**: .NET $DOTNET_VERSION, $OS_NAME $ARCH
...
echo "[benchmark-summary] ✅ Generated: $OUTPUT_FILE" >&2
echo "[benchmark-summary] Metadata updated with commit $COMMIT on $DATE" >&2

# Note: Future enhancement could parse JSON results to extract actual metrics
# For now, we maintain the template structure with representative data
echo "[benchmark-summary] Note: Metrics are template-based. Future: parse JSON for live data." >&2
