...
public sealed class ChessMoveGenerator
{
    ...
    public IReadOnlyCollection<PseudoMove> Generate(GameState state)
    {
        ...
        var pieceStates = GetPlayerPieceStates(state, activePlayer);

        foreach (var pieceState in pieceStates)
        {
            if (pieceState.IsCaptured)
            {
                continue;
            }
    ...
    {
        var activePlayerStates = state.GetStates<ActivePlayerState>();

        foreach (var activePlayerState in activePlayerStates)
        {
            if (activePlayerState.IsActive)
            {
                return activePlayerState.Player;
            }
        }
    ...
    private IEnumerable<PieceState> GetPlayerPieceStates(GameState state, Player player)
    {
        ...
        foreach (var pieceState in pieceStates)
        {
            if (pieceState.Piece.Owner == player)
            {
                result.Add(pieceState);
            }
        }
    ...
    private bool IsOccupied(GameState state, Tile tile)
    {
        ...
        foreach (var pieceState in pieceStates)
        {
            if (!pieceState.IsCaptured && pieceState.Tile == tile)
            {
                return true;
            }
        }
    ...
    private bool IsOccupiedByFriendly(GameState state, Tile tile, Player player)
    {
        ...
        foreach (var pieceState in pieceStates)
        {
            if (!pieceState.IsCaptured && pieceState.Tile == tile && pieceState.Piece.Owner == player)
            {
                return true;
            }
        }
    ...
    {
        var relations = from.GetRelations(direction);
