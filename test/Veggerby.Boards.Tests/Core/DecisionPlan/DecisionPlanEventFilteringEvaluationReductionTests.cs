...
/// <summary>
/// Tests demonstrating quantitative reduction in rule evaluations when event kind filtering is enabled.
/// </summary>
public class DecisionPlanEventFilteringEvaluationReductionTests
{
    ...
    public void GivenMoveEvent_FilteringDoesNotIncreaseEvaluations()
    {
        ...
        // assert
        // Implementation classifies event kind prior to condition evaluation even when filtering flag disabled; thus no evaluation delta.
        enabledObs.Evaluated.Should().Be(disabledObs.Evaluated);
        // Current engine does not emit EventKindFiltered skips for non-matching kinds in grouped traversal (pre-discrimination suppresses skip notification).
