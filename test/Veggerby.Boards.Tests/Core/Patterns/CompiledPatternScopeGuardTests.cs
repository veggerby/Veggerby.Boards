...
public class CompiledPatternScopeGuardTests
{
    private static CompiledPiecePatterns CompileFor(params IPattern[] patterns)
    {
        ...
        var b = new Tile("b");
        var c = new Tile("c");
        var dAB = new Direction("ab");
        var dBC = new Direction("bc");
        var rAB = new TileRelation(a, b, dAB);
        var rBC = new TileRelation(b, c, dBC);
    ...
    public void GivenTinyBoard_WithAllSupportedKinds_WhenResolving_ThenCompiledMatchesLegacy()
    {
        ...
        var dab = new Direction("ab");
        var dbc = new Direction("bc");
        var dad = new Direction("ad");
        var rAB = new TileRelation(a, b, dab);
        var rBC = new TileRelation(b, c, dbc);
        var rAD = new TileRelation(a, d, dad);
        ...
        TilePath? legacyFixed = null;
        TilePath? legacyRay = null;
        TilePath? legacyMulti = null;
        foreach (var target in new[] { c, b, d })
        {
            ...
            {
                pat.Accept(visitor);
                if (visitor.ResultPath is not null && visitor.ResultPath.To.Equals(target))
                    break;
            }
            if (target.Equals(c))
                legacyFixed = visitor.ResultPath;
            else if (target.Equals(b))
                legacyRay = visitor.ResultPath;
            else if (target.Equals(d))
                legacyMulti = visitor.ResultPath;
        }
