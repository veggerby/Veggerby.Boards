...
internal sealed class NoOpRule : Flows.Rules.IGameEventRule
{
    public ConditionResponse Check(GameEngine engine, GameState gameState, Flows.Events.IGameEvent @event)
    {
        return ConditionResponse.Valid; // Always allow; we are not exercising rule evaluation here.
    }

    public GameState HandleEvent(GameEngine engine, GameState gameState, Flows.Events.IGameEvent @event)
    {
